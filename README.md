...
- **任务驱动路由引擎 (v1.0 Preview)**: 基于 YAML 配置的灵活分发系统，支持多源(Source) -> 关键词(Keyword) -> 多目标(Output) 的智能路由。
...
- [ ] **v1.0 (TG-Link-Dispatcher)**: **全面重构为配置驱动的任务调度引擎**。支持多源、多目标路由，灵活的 YAML 规则配置，以及基于关键词的智能分发。
- [ ] **v1.1 (LTS)**: 长期支持版本、多数据库后端 (SQLite/PostgreSQL) 选配。
